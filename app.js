...
// CSV 2: Quiz responses — one row per learner per question
async function exportAllQuizCSV() {
  const stats = getAllStats();
  if (!stats.length) { toast('No learners to export.', 'err'); return; }

  const rows = [['Learner','Email','Subject','Course','Quiz','Question #','Question','Learner Answer','Correct Answer','Result','Quiz Score %']];
  ...
  for (const u of stats) {
    async function emitQuiz(c, quizArr, quizLabel, score, qKey) {
      ...
      quizArr.forEach((q, i) => {
        if (score == null) {
          rows.push([u.name, u.email, u.dept||'', c.title, quizLabel, i+1, q.q, 'Not attempted', q.opts[q.ans], '—', '—']);
          ...
          rows.push([u.name, u.email, u.dept||'', c.title, quizLabel, i+1, q.q, learnerAns, q.opts[q.ans], correct?'Correct':'Incorrect', score+'%']);
        } else {
          rows.push([u.name, u.email, u.dept||'', c.title, quizLabel, i+1, q.q, '(detail not available)', q.opts[q.ans], '—', score+'%']);
        }
      });
    }
